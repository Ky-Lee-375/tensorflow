...
#                                        run_metadata=run_metadata)
  ...
  if run_metadata is None:
    print("does it still work ? 22222!!!!!")
  if run_metadata is not None:
    print("does it still work ?")
    jsonObj = MessageToJson(run_metadata)
    #with open('/home/uh30/inception_logs/im_size_224_1gpuforce_1000iters/metadata_%d.json' % (np_global_step), 'w') as outfile:
    with open('%s/metadata_%d.json' % (log_fn_pref, np_global_step), 'w') as outfile:
      json.dump(jsonObj, outfile)

    tl = timeline.Timeline(run_metadata.step_stats)
    trace = tl.generate_chrome_trace_format()

#    trace_file = open('/home/uh30/inception_logs/im_size_224_1gpuforce_1000iters/timeline_%d.ctf.json' % (np_global_step), 'w')
    trace_file = open('%s/timeline_%d.ctf.json' % (log_fn_pref, np_global_step), 'w')
...
def train(train_op,
          ...
          ignore_live_threads=False, log_fn_pref=None, doLogs=None):
